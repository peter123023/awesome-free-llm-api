...
| [Onomeo](https://onomeo.com/zh) | 签到制每日额度(`z-ai/glm-5.3-flash-free`,195 额度/次,约 1,025 次/天) | 优势:国内直连、OpenAI 兼容、1M 上下文;限制:非免费档、需每日手动签到、官方自曝可用率约 55%、公测聚合平台随时可能变 |  |
...
> OpenAI 旗舰模型（`gpt-6-astra`）

| 免费渠道 | 免费形式 | 优势 / 限制 | API 质量 |
|------|------|------|------|
| [Onomeo](https://onomeo.com/zh) | 签到制每日额度(47,100 额度/次,约 4 次/天) | 优势:国内直连、OpenAI 兼容、旗舰款也被额度覆盖;限制:非免费档、每日额度极少、需每日手动签到、可用率约 66%、公测聚合平台随时可能变 |  |
...
- **可用模型（需付费 credits 解锁）**：deepseek-v4.1-flash（$0.15/$0.60 闲时）、deepseek-v4-pro、deepseek-v4-flash（$0.22/$0.66 闲时）、glm-5.1 / 5.2 / 5.3 / 5.3-flash、kimi-k2.6 / k2.7-code / k3、minimax-m2.7 / m3、qwen3.5:397b、gpt-oss:20b / 120b、gemma4、nemotron-3-nano / super / ultra、mistral-large-3（云目录共 20 款，**定价页 20 款全部标价、无一为 $0**）
...
- **状态**：Active（**未列入任何模型免费索引**）— 核实于 2026-09-12（⚠️ **重要更正**：本条目此前被写成「V4.1 Flash / V4 Flash 等多个模型的免费入口」属于误读——`/api/tags` 返回 20 款只是**云目录**，不等于免费清单。实测无 API Key 调用 `deepseek-v4.1-flash` 直接返回 Unauthorized；定价页 20 款**全部按量标价、无一款 $0**（V4 Flash $0.22/$0.66）；FAQ 明示 Free 档仅"a starter amount of usage for a smaller set of starter models"，且"Buying usage credits unlocks all models"，而 **starter 清单从未公布，亦无任何 endpoint 或目录标记可查**。因此**无法证实任一具体模型在免费档内**，本仓库不把任何模型记为「Ollama Cloud 免费」。免费档真实价值＝少量未公开额度 + 未公开的 starter 小模型；Free 档 1 并发，额度按月从注册日重置、不结转）
...
- **免费形式**：签到制每日额度（非免费档模型）——每日签到最高领 200,000 额度（连续签到一周达上限），完成问卷、邀请好友各得 50,000（双方）；会员 $5/月每月额外 10,000,000 额度
- **网页说明**：中文聚合网关（公测），一个密钥调用 54 个模型，OpenAI 兼容；官方自述「部分功能仍在完善」
...
- **「签到领额度」不是「免费档模型」。** Onomeo 这类聚合平台用签到/问卷换每日额度——全目录都能调但消耗差异极大（旗舰款一次烧数千额度，一天只有几次），额度当日有效、需每天手动领取，且公测平台随时可能改规则。
